...
const sendAwardNotificationEmails = async (awardedBids) => {
    ...
    for (const vendorId in notificationsByVendor) {
        ...
        const htmlBody = `<div style="font-family: Arial, sans-serif; max-width: 800px; margin: auto; border: 1px solid #ddd; border-radius: 8px;">
...
// FIX: Updates the bid amount with the final negotiated rate upon awarding
apiRouter.post('/contracts/award', authenticateToken, isAdmin, async (req, res, next) => {
    let connection;
    try {
        ...
        const [users] = await dbPool.query('SELECT user_id, full_name, email FROM users WHERE user_id IN (?)', [vendorIds]);
        const vendorInfoMap = new Map(users.map(user => [user.user_id, { trucker_name: user.full_name, trucker_email: user.email }]));
        const bidsForEmail = bids.map(bid => ({ ...bid, ...vendorInfoMap.get(bid.vendor_id) }));

        await connection.beginTransaction();
        for (const bid of bids) {
            // Step 1: Update the bids table with the final negotiated amount. THIS IS THE FIX.
            await connection.query(
                "UPDATE bids SET bid_amount = ? WHERE load_id = ? AND vendor_id = ?",
                [bid.final_amount, bid.load_id, bid.vendor_id]
            );

            // Step 2: Clear any previous awards for this load
            await connection.query("DELETE FROM awarded_contracts WHERE load_id = ?", [bid.load_id]);

            // Step 3: Insert the new awarded contract
            await connection.query(
                "INSERT INTO awarded_contracts (load_id, requisition_id, vendor_id, awarded_amount, remarks, awarded_date) VALUES (?, ?, ?, ?, ?, NOW())",
                [bid.load_id, bid.requisition_id, bid.vendor_id, bid.final_amount, bid.remarks]
            );

            // Step 4: Update the load status to 'Awarded'
...
apiRouter.get('/loads/assigned', authenticateToken, async (req, res, next) => {
    try {
        ...
        let query = `
            SELECT 
                ...
                CASE 
                    WHEN b.bid_id IS NOT NULL THEN (SELECT COUNT(DISTINCT b2.vendor_id) + 1 FROM bids b2 WHERE b2.load_id = tl.load_id AND b2.bid_amount < b.bid_amount) 
                    ELSE NULL 
                END AS my_rank 
            FROM truck_loads tl 
        ...
        if (startDate) { query += ` AND tl.requirement_date >= ?`; params.push(startDate); }
        if (endDate) { query += ` AND tl.requirement_date <= ?`; params.push(endDate); }
        query += ` ORDER BY tl.requirement_date ASC, tl.load_id DESC`;
...
apiRouter.get('/trucker/bidding-history', authenticateToken, async (req, res, next) => {
    try {
        ...
        let query = `
            SELECT 
                ...
                CASE WHEN tl.status = 'Awarded' THEN (SELECT MIN(b3.bid_amount) FROM bids b3 WHERE b3.load_id = bhl.load_id) ELSE NULL END as l1_bid 
        ...
        if (startDate) { query += ' AND DATE(bhl.submitted_at) >= ?'; params.push(startDate); }
        if (endDate) { query += ' AND DATE(bhl.submitted_at) <= ?'; params.push(endDate); }
...
apiRouter.post('/loads/approve', authenticateToken, isAdmin, async (req, res, next) => {
    let connection;
    try {
        connection = await dbPool.getConnection();
        const { approvedLoadIds, truckerAssignments, requisitionId, biddingStartTime, biddingDurationMinutes } = req.body;
        
        if (!biddingStartTime || !biddingDurationMinutes) {
            return res.status(400).json({ success: false, message: 'Bidding Start Time and Duration are mandatory.' });
        }

        const startTime = new Date(biddingStartTime);
        const endTime = new Date(startTime.getTime() + parseInt(biddingDurationMinutes, 10) * 60000);

        await connection.beginTransaction();
        if (approvedLoadIds && approvedLoadIds.length > 0) {
            await connection.query("UPDATE truck_loads SET status = 'Active', bidding_start_time = ?, bidding_end_time = ? WHERE load_id IN (?)", [startTime, endTime, approvedLoadIds]);
        }
...
apiRouter.get('/admin/awarded-contracts', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const { startDate, endDate } = req.query;
        let query = `
            SELECT 
                ...
                ac.awarded_amount, 
                ac.awarded_date, 
                u.full_name as trucker_name, 
                u.email as trucker_email, 
                u.contact_number as trucker_contact, 
                tl.loading_point_address, 
                tl.unloading_point_address, 
                tl.requirement_date, 
                tl.approx_weight_tonnes, 
        ...
        const params = [];
        const whereClauses = [];
        if (startDate) { whereClauses.push('DATE(ac.awarded_date) >= ?'); params.push(startDate); }
        if (endDate) { whereClauses.push('DATE(ac.awarded_date) <= ?'); params.push(endDate); }
...
apiRouter.get('/admin/all-loads', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const { status, startDate, endDate } = req.query;
        let query = `
            SELECT 
        ...
        const params = [];
        const whereClauses = [];
        if (status) { whereClauses.push('tl.status = ?'); params.push(status); }
        if (startDate) { whereClauses.push('tl.requirement_date >= ?'); params.push(startDate); }
        if (endDate) { whereClauses.push('tl.requirement_date <= ?'); params.push(endDate); }
        if (whereClauses.length > 0) { query += ` WHERE ${whereClauses.join(' AND ')}`; }
        query += ' ORDER BY tl.load_id DESC';
        const [loads] = await dbPool.query(query, params);
        res.json({ success: true, data: loads });
    } catch (error) {
        next(error);
...
apiRouter.get('/admin/bidding-history', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const { startDate, endDate } = req.query;
        let query = `
            SELECT 
        ...
        const params = [];
        const whereClauses = [];
        if (startDate) { whereClauses.push('DATE(bhl.submitted_at) >= ?'); params.push(startDate); }
        if (endDate) { whereClauses.push('DATE(bhl.submitted_at) <= ?'); params.push(endDate); }
        if (whereClauses.length > 0) { query += ` WHERE ${whereClauses.join(' AND ')}`; }
        query += ' ORDER BY bhl.submitted_at DESC';
        const [bids] = await dbPool.query(query, params);
        res.json({ success: true, data: bids });
...
apiRouter.post('/admin/reports-data', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const { startDate, endDate } = req.body;
        const params = [];
        let whereClause = '';
        if (startDate && endDate) { whereClause = ' WHERE ac.awarded_date BETWEEN ? AND ?'; params.push(startDate, `${endDate} 23:59:59`); }
        const queries = { 
            detailedReport: `
                SELECT 
                    ac.load_id, 
            ...
            kpis: `SELECT COALESCE(SUM(ac.awarded_amount), 0) AS totalSpend, COUNT(ac.load_id) as awardedLoads FROM awarded_contracts ac ${whereClause}`, 
            topTruckers: `SELECT u.full_name as label, COUNT(ac.load_id) as value FROM awarded_contracts ac JOIN users u ON ac.vendor_id = u.user_id ${whereClause} GROUP BY label ORDER BY value DESC LIMIT 5`, 
            spendOverTime: `SELECT DATE_FORMAT(ac.awarded_date, '%Y-%m-%d') as label, SUM(ac.awarded_amount) as value FROM awarded_contracts ac ${whereClause} GROUP BY label ORDER BY label`, 
            spendByMaterial: `SELECT im.item_name as label, SUM(ac.awarded_amount) as value FROM awarded_contracts ac JOIN truck_loads tl ON ac.load_id = tl.load_id JOIN item_master im ON tl.item_id = im.item_id ${whereClause} GROUP BY label ORDER BY value DESC LIMIT 5` 
        };
        const [ [detailedReport], [[kpisResult]], [topTruckers], [spendOverTime], [spendByMaterial] ] = await Promise.all([ dbPool.query(queries.detailedReport, params), dbPool.query(queries.kpis, params), dbPool.query(queries.topTruckers, params), dbPool.query(queries.spendOverTime, params), dbPool.query(queries.spendByMaterial, params) ]);
        const kpis = kpisResult || { totalSpend: 0, awardedLoads: 0 };
...
apiRouter.post('/loads/bulk-approve', authenticateToken, isAdmin, async (req, res, next) => {
    let connection;
    try {
        connection = await dbPool.getConnection();
        const { loadIds, truckerIds, biddingStartTime, biddingDurationMinutes } = req.body;
        
        if (!loadIds || loadIds.length === 0) {
            return res.status(400).json({ success: false, message: 'Please select at least one load to approve.' });
        }
        if (!truckerIds || truckerIds.length === 0) {
            return res.status(400).json({ success: false, message: 'Please assign at least one trucker.' });
        }
        if (!biddingStartTime || !biddingDurationMinutes) {
            return res.status(400).json({ success: false, message: 'Bidding Start Time and Duration are mandatory.' });
        }
        ...
        if (requisitionIds.length > 0) {
             // Update requisitions
            await connection.query("UPDATE requisitions SET status = 'Processed', approved_at = NOW() WHERE requisition_id IN (?)", [requisitionIds]);

            // Update trucker assignments for each requisition
            for (const reqId of requisitionIds) {
                await connection.query('DELETE FROM trucker_assignments WHERE requisition_id = ?', [reqId]);
                const assignmentValues = truckerIds.map(vId => [reqId, vId, new Date()]);
                await connection.query('INSERT INTO trucker_assignments (requisition_id, vendor_id, assigned_at) VALUES ?', [assignmentValues]);
            }
        }
        
        await connection.commit();
        res.json({ success: true, message: `${loadIds.length} load(s) approved and assigned successfully!` });
    } catch (error) {
        if (connection) await connection.rollback();
        next(error);
    } finally {
...
apiRouter.get('/conversations', authenticateToken, async (req, res, next) => { try { const myId = req.user.userId; const [users] = await dbPool.query('SELECT user_id, full_name, role FROM users WHERE user_id != ? AND is_active = 1', [myId]); if (users.length === 0) return res.json({ success: true, data: [] }); const userMap = new Map(users.map(u => [u.user_id, { user_id: u.user_id, full_name: u.full_name, role: u.role === 'Vendor' ? 'Trucker' : (u.role === 'User' ? 'Shipper' : u.role), last_message: null, last_message_timestamp: null, last_message_status: null, last_message_sender: null, unread_count: 0 }])); const otherUserIds = Array.from(userMap.keys()); const lastMessagesQuery = `SELECT * FROM messages m WHERE m.message_id IN ( SELECT MAX(message_id) FROM messages WHERE (sender_id = ? AND recipient_id IN (?)) OR (recipient_id = ? AND sender_id IN (?)) GROUP BY LEAST(sender_id, recipient_id), GREATEST(sender_id, recipient_id) )`; const [lastMessages] = await dbPool.query(lastMessagesQuery, [myId, otherUserIds, myId, otherUserIds]); const unreadQuery = `SELECT sender_id, COUNT(*) as count FROM messages WHERE recipient_id = ? AND status != 'read' GROUP BY sender_id`; const [unreadCounts] = await dbPool.query(unreadQuery, [myId]); lastMessages.forEach(msg => { const otherUserId = msg.sender_id == myId ? msg.recipient_id : msg.sender_id; if (userMap.has(otherUserId)) { const user = userMap.get(otherUserId); user.last_message = msg.message_body; user.last_message_timestamp = msg.timestamp; user.last_message_status = msg.status; user.last_message_sender = msg.sender_id; } }); unreadCounts.forEach(uc => { if (userMap.has(uc.sender_id)) { userMap.get(uc.sender_id).unread_count = uc.count; } }); const sortedUsers = Array.from(userMap.values()).sort((a, b) => (new Date(b.last_message_timestamp) || 0) - (new Date(a.last_message_timestamp) || 0)); res.json({ success: true, data: sortedUsers }); } catch (e) { next(e); }});
