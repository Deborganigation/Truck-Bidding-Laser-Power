    ...
    try {
        ...
        for (const bid of bids) {
            ...
            const [[loadDetails]] = await connection.query(
                `SELECT status, bidding_start_time, bidding_end_time, NOW() as current_server_time, (NOW() >= bidding_start_time OR bidding_start_time IS NULL) as is_after_start, (NOW() <= bidding_end_time OR bidding_end_time IS NULL) as is_before_end FROM truck_loads WHERE load_id = ?`,
                [bid.loadId]
            );
            ...
            if (!(loadDetails.is_after_start && loadDetails.is_before_end)) {
                console.log('--- BIDDING WINDOW CHECK FAILED ---');
                console.log(`Load ID: ${bid.loadId}`);
                console.log(`Current Server Time (IST): ${loadDetails.current_server_time}`);
                console.log(`Bidding Start Time (DB):   ${loadDetails.bidding_start_time}`);
                console.log(`Bidding End Time (DB):     ${loadDetails.bidding_end_time}`);
                console.log(`Check 1 (is_after_start):  ${loadDetails.is_after_start}`);
                console.log(`Check 2 (is_before_end):   ${loadDetails.is_before_end}`);
                console.log('------------------------------------');
