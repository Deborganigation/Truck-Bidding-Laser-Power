...
const fs = require('fs/promises'); // <-- Naya module add kiya hai
        ...
        for (const row of jsonData) {
            ...
            try {
                await connection.query(
                    ...
                    [reqId, userId, row.LoadingPoint, row.UnloadingPoint, itemId, row.WeightInTonnes, row.RequirementDate, row.InhouseRequestionNo, row.Remarks]
                );
                successCount++;
            } catch (dbError) {
...
// ... baaki saare purane routes waise hi rahenge ...
// Yahan par bas naye/badle hue routes dikha raha hoon
// Aapko apne code mein sirf inhi ko badalna hai ya add karna hai

// === YEH ROUTE PURA REPLACE KAREIN ===
apiRouter.post('/loads/bulk-upload', authenticateToken, isAdmin, upload.single('bulkFile'), async (req, res, next) => {
    ...
    try {
        ...
        const [historyResult] = await connection.query(
            `INSERT INTO bulk_upload_history (file_name, total_rows, status, started_at, uploaded_by_user_id) VALUES (?, ?, 'PROCESSING', NOW(), ?)`,
            [originalFileName, totalRows, req.user.userId]
        );
        uploadId = historyResult.insertId;

        // Process in background
        processBulkUpload(uploadId, jsonData, req.user.userId);

        res.status(202).json({ success: true, message: 'File upload started. You can check the status in the Bulk Upload History page.' });

    } catch (error) {
        if (uploadId && connection) {
             await connection.query("UPDATE bulk_upload_history SET status = 'FAILED' WHERE upload_id = ?", [uploadId]);
        }
        next(error);
    } finally {
        if (connection) connection.release();
...
// === YEH 2 NAYE ROUTES ADD KAREIN ===
apiRouter.get('/admin/bulk-upload-history', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        const [history] = await dbPool.query(`
            SELECT h.*, u.full_name as uploaded_by_name 
            FROM bulk_upload_history h 
            LEFT JOIN users u ON h.uploaded_by_user_id = u.user_id 
...
apiRouter.get('/admin/download-error-file/:uploadId', authenticateToken, isAdmin, async (req, res, next) => {
    try {
        ...
        const [[history]] = await dbPool.query("SELECT error_file_path FROM bulk_upload_history WHERE upload_id = ?", [uploadId]);

        if (!history || !history.error_file_path) {
            return res.status(404).send('Error file not found.');
        }

        const filePath = path.join(__dirname, history.error_file_path);
...
// Puraane `/loads/bulk-upload` route ko hata kar, upar diye gaye naye code se replace karein
// Aur upar diye gaye 2 naye routes add karein. Baaki sab same rahega.
// Yahan neeche main aapka baaki ka code copy kar raha hoon taaki file complete ho.

// PASTE YOUR OTHER ROUTES HERE...
